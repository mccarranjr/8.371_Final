...
def snapshot_state(b_list = None, obs_list = None, im_time_evo_op = None):
    ...
    return im_time_evo_op@rho_snapshot@im_time_evo_op.conj().T
...
def plot_energy_error(time_steps = None, exact_energy = None, hamiltonian = None):
    ...
    with open('Data/ITE_data.csv', 'w') as file:
        ...
        for step in range(time_steps):
            ...
            error = np.sqrt((energy-exact_energy).conj()*(energy-exact_energy))
            out.append(error)

            file.write(f'{step},{error.real}\n')

    plt.plot(range(time_steps), out)
    plt.xlabel('Time Steps')
    plt.ylabel('Energy Error')
    plt.title('Energy Error over Time')
    plt.savefig("ITE_energy_error_plot.png")
    plt.show()

def single_application(circuit_template, params, shadow_size, num_qubits):

    shadow = calculate_classical_shadow( 
        circuit_template = circuit_template, params = params, 
...
def plot_log_energy_error():
    df = pd.read_csv('Data/ITE_data.csv',names=['Time Steps','Energy Error'])
    print(df.info())
    plt.plot(df['Time Steps'],df['Energy Error'])

    plt.xlabel('Time Steps')
    plt.ylabel('Log Energy Error')
    plt.title('Log Energy Error Over Time')
    plt.yscale('log')
    plt.savefig('ITE_Log_energy_error')
    plt.show()


    
#plot_energy_error(time_steps=time_steps, exact_energy=exact_energy,hamiltonian=hamiltonian)
plot_log_energy_error()
